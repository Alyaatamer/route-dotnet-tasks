...
namespace Task1
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var res1 = ProductList.Where(n => n.UnitPrice > 150);

            var res2 = from c in CustomerList
                       from o in c.Orders
                       select o;

            var res3 = CustomerList.OrderBy(c => c.CustomerName).ToList().Select(c => c.CustomerName);

            var res4 = ProductList.All(n => n.UnitsInStock > 0);
            ...
            {
                Console.WriteLine("The store is not full");
            }

            var res5 = (from c in CustomerList
                       from o in c.Orders
                       where o.Total>200
                       select o).FirstOrDefault();

            var res6 = CustomerList.SelectMany(c => c.Orders).Where(c=> c.Total>200).FirstOrDefault();
